...
import type { ApiErrorResponse, Movie } from "../../lib/contracts";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Movie[] | ApiErrorResponse>) {
  ...
  try {
    ...
    res.status(500).json({ message: "Failed to load Star Wars films." });
  }
}
